...
class ActualViewController: UIViewController, NSURLConnectionDelegate, WKUIDelegate, WKNavigationDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let redirectUrlPath = (UIApplication.shared.delegate as! AppDelegate).redirecrUrl {
            ...
            let request = URLRequest.init(url: URL.init(string: "\(URL_API_BASE)/service_record")!)
        ...
        afManager.delegate.taskWillPerformHTTPRedirection = {session, task, response, request in
            ...
            self.afManager.request(URL.init(string: redirectFullLocation)!).response(completionHandler: { (res) in
                ...
                if res.response!.statusCode == 200 {
                    ...
                    self.initWebView()
                    _ = self.wkWebView.load(URLRequest.init(url: URL.init(string: "\(URL_API_BASE)/service_record")!))
                } else {
                    self.showExpireTokenDialog()
                }
            })
...
extension ActualViewController : SlideMenuControllerDelegate, UIDocumentInteractionControllerDelegate {
